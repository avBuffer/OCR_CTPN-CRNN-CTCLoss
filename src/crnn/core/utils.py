...
def train(net, train_data, valid_data, num_epochs, optimizer,
          criterion, saver_freq=50, saver_prefix='vgg16'):
    ...
    for epoch in range(num_epochs):
        ...
        if valid_data is not None:
            ...
            if valid_acc / len(valid_data) > best_acc:
                ...
                torch.save(net.state_dict(), 'models/{}-{}-{}-0819-model-db.pth'.format(
    ...
    def encode(self, text):
        ...
        try:
            if isinstance(text, str):
                # for char in text:
                #     print(char)
                text = [self.dict[ord(char.lower() if self._ignore_case else char)] for char in text] # if char in self.dict.keys()
                length = [len(text)]
    ...
    def decode(self, t, length, raw=False):
        ...
        if length.numel() == 1:
            ...
            assert t.numel() == length, \
               "text with length: {} does not match declared length: {}".format(t.numel(), length)
            if raw:
...
def prettyPrint(v):
    ...
    print('| Max: %f | Min: %f | Mean: %f' % (v.max().item(), v.min().item(), v.mean().item()))
