...
def rename(filepath):
    ...
    for idx, filename in enumerate(filelist):
        ...
        shutil.move(filepath + '/' + filename, filepath + '/' + str(idx).zfill(3) + '.' + ext)
...
def zlog(func):
    def new_fn(*args):
        ...
        # print(f'{fulltime} {__file__} {func.__name__}:{inspect.getsourcelines(func)[-1]} \
        #       cost: {duration}s', sep=' ', end='\n', file=sys.stdout, flush=False)        
        return result
    return new_fn
...
def pil2cv(image):
    # assert isinstance(image, PIL.Image.Image) or isinstance(image, PIL.JpegImagePlugin.JpegImageFile), \
    #    f'input image type is not PIL.image and is {type(image)}'
...
if __name__ == '__main__':
    ...
    allimg = getlabeljson('all_img.json')
