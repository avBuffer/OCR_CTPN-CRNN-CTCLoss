...
import abc
import trans_utils
from PIL import Image, ImageDraw, ImageFont, ImageChops, ImageEnhance, ImageFilter, ImageOps
...
class AdjustResolution(TransBase):
    ...
    def tranfun(self, image):
        image = getpilimage(image)
        w, h = image.size
        rate = np.random.random() * (self.max_rate - self.min_rate) + dself.min_rate
        w2 = int(w * rate)
        h2 = int(h * rate)
