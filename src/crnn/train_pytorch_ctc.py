...
config.train_infofile = 'data/train.txt'
config.val_infofile = 'data/train.txt'
...
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=config.batchSize, shuffle=True,
                                           sampler=sampler, num_workers=int(config.workers),
                                           collate_fn=mydataset.alignCollate(imgH=config.imgH, imgW=config.imgW, keep_ratio=config.keep_ratio))
test_dataset = mydataset.MyDataset(info_filename=config.val_infofile, transform=mydataset.resizeNormalize((config.imgW, config.imgH), is_test=True))
...
def val(net, dataset, criterion, max_iter=100):
    ...
    if accuracy > best_acc:
        best_acc = accuracy
        torch.save(crnn.state_dict(), '{}/{}_{}_{}.pth'.format(config.saved_model_dir, config.saved_model_prefix, epoch, int(best_acc * 1000)))
