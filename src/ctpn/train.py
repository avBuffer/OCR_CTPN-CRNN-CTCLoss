...
import core.config
from core.model import CTPN_Model, RPN_CLS_Loss, RPN_REGR_Loss
from core.dataset import ICDARDataset
...
def save_checkpoint(state, epoch, loss_cls, loss_regr, loss, ext='pth'):
    ...
    check_path = os.path.join(config.checkpoints_dir,
                              f'v3_ctpn_ep{epoch:02d}_{loss_cls:.4f}_{loss_regr:.4f}_{loss:.4f}.{ext}')
    try:
        torch.save(state, check_path)
    except BaseException as e:
        print('fail to save to {}'.format(check_path), ' e:', e)
    ...
    for epoch in range(resume_epoch + 1, epochs):
        ...
        if best_loss_cls > epoch_loss_cls or best_loss_regr > epoch_loss_regr or best_loss > epoch_loss:
            ...
            save_checkpoint({'model_state_dict': best_model.state_dict(), ' epoch': epoch},
                            epoch, best_loss_cls, best_loss_regr, best_loss)
