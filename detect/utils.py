...
def cal_iou(box1, box1_area, boxes2, boxes2_area):
    """box1 [x1,y1,x2,y2], boxes2 [Msample,x1,y1,x2,y2]"""
...
def cal_overlaps(boxes1, boxes2):
    """boxes1 [Nsample,x1,y1,x2,y2] anchor, boxes2 [Msample,x1,y1,x2,y2] grouth-box"""
...
def cal_rpn(imgsize, featuresize, scale, gtboxes):
    ...
    outside_anchor = np.where((base_anchor[:, 0] < 0) | (base_anchor[:, 1] < 0) |
                              (base_anchor[:, 2] >= imgw) | (base_anchor[:, 3] >= imgh))[0]
    ...
    def build_graph(self, text_proposals, scores, im_size):
        ...
        for index, box in enumerate(text_proposals):
            ...
            if self.is_succession_node(index, succession_index):
                # NOTE: a box can have multiple successions(precursors) if multiple successions(precursors) have equal scores.
